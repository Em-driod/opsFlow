...
export const getClientById = async (req: Request, res: Response) => {
    try {
        const client = await Client.findOne({ _id: req.params.id, businessId: (req.user as any).businessId }).populate("transactions"); // Filter by businessId
        if (client) {
            res.json(client);
        } else {
...
export const updateClient = async (req: Request, res: Response) => {
    try {
        const client = await Client.findOne({ _id: req.params.id, businessId: (req.user as any).businessId }); // Filter by businessId
        if (client) {
